...
const ViewListTask = ({ task }) => {
  ...
  return (
    <div
      className={`bg-tdColor bg-opacity-15 grid md:grid-cols-6 sm:grid-cols-2 gap-4 items-center rounded-md px-4 py-3  text-[14px] cursor-pointer hover:shadow-lg transform hover:scale-[103%] duration-300 ease-out ${
        task.estado == 4 && "border-l-[6px] border-[#E74C3C]"
      }`}
    >
      <div className="md:col-span-1 sm:col-span-2">000{task.id}</div>
      <div className="md:col-span-2 sm:col-span-2">
        Aberto por
        <strong className="text-primaryColor"> {task.user.name}</strong>,
        atribuido a
        <strong className="text-primaryColor"> {task.clients.name} </strong>-{" "}
